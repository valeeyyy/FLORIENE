...
<head>
  ...
  <link
    href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@700&family=Inter:wght@300;400;500;600;700;800&display=swap"
    rel="stylesheet">
    ...
    <div class="grid grid-cols-1 min-[900px]:grid-cols-2 gap-5">
      ...
      <div class="profile-card">
        ...
        <div class="profile-info-row">
          ...
          <span
            class="profile-info-val"><?= $user['nomor_hp'] ? htmlspecialchars($user['nomor_hp']) : 'Not set' ?></span>
        </div>
        <div class="profile-info-row">
        ...
        <div style="margin-top:18px;">
          <button class="btn btn-pink" style="width:100%;justify-content:center;" onclick="openEditModal()">✏️ Edit
            Profile</button>
        </div>
      </div>
      ...
      <div class="profile-card">
        ...
        <?php if ($poin >= 10): ?>
          <div style="text-align:center; margin-top:10px; font-size:13px; font-weight:700; color:var(--pink-dark);">🎁
            <?= floor($poin / 10) ?> reward(s) ready</div>
        <?php endif; ?>
        <div
          style="margin-top:18px; padding:14px; background:var(--pink-bg); border-radius:10px; border:1px solid var(--border);">
          <div style="font-size:12px; font-weight:700; color:var(--dark); margin-bottom:6px;">🎁 Reward</div>
          <div style="font-size:12px; color:var(--muted); line-height:1.7;">Collect <strong>10 points</strong> and get
            <strong>1 FREE Bloomies Keychain!</strong><br><span style="font-size:11px;">Points are credited
              automatically when your order is completed by admin.</span></div>
          <?php if (!$pending && $siap_redeem): ?>
            <button class="btn btn-pink" style="width:100%;justify-content:center;margin-top:10px;"
              onclick="openRedeemModal()">🎁 Redeem Reward</button>
          <?php endif; ?>
        </div>
        <?php if ($pending): ?>
          <div style="margin-top:12px; padding:12px; background:#fff3cd; border-radius:8px; border:1px solid #ffeaa7;">
            ...
            <div style="font-size:11px; color:#856404; margin-top:4px;">Color:
              <strong><?= htmlspecialchars($pending['warna_bloomies']) ?></strong></div>
          </div>
        <?php endif; ?>
        <?php if ($loyalty): ?>
          <div style="margin-top:12px; font-size:11px; color:var(--muted); text-align:center;">Card ID:
            <?= (int) $loyalty['id_loyalty_card'] ?></div>
        <?php endif; ?>
      </div>
        ...
        <div class="order-list">
          <?php if (empty($orders)): ?>
            <div style="text-align:center; padding:28px; color:var(--muted); font-size:14px;">
              No orders yet. <a href="index.html#pesan" style="color:var(--pink-dark);font-weight:700;">Place your first
                order!</a>
            </div>
          <?php else:
            foreach ($orders as $o): ?>
              <div class="order-item">
                <div class="order-item-left">
                  ...
                  <div class="order-date"><?= fmt_date($o['created_at']) ?> · Rp
                    <?= number_format($o['total_harga'], 0, ',', '.') ?></div>
                </div>
      ...
      <div class="profile-card">
        ...
        <div class="delete-zone">
          <p>Deleting your account is <strong>permanent</strong> and cannot be undone. All your data including order
            history and loyalty points will be permanently removed.</p>
          <form method="POST" action="php/akun.php"
            onsubmit="return confirm('Delete your account permanently? All your data and order history will be removed. This cannot be undone.');">
            <input type="hidden" name="aksi" value="delete">
            <button type="submit" class="btn btn-danger" style="width:100%;justify-content:center;">🗑️ Delete My
              Account</button>
          </form>
        </div>
      </div>
      ...
      <form id="form-redeem" method="POST" action="php/loyalty.php" novalidate style="text-align:left;">
        ...
        <div style="margin:16px 0; padding:12px; background:#e8f5e9; border-radius:8px; border-left:3px solid #4caf50;">
          <div style="font-size:12px; color:#2e7d32; line-height:1.6;">
            ...
            🎁 <strong>10 points</strong> will be deducted right away. Any extra points stay on your card for the next
            reward.
          </div>
        </div>
        <div class="fl-modal-btns" style="margin-top:20px;">
